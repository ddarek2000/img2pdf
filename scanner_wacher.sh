...
/usr/bin/docker run --rm -v "$SCANNER_BASE:/home/docker" img2pdf magick convert -flatten -fuzz 5% -fill "#ffffff" -opaque "#f2f2f2" -tr
im +repage -density 300 -depth 8 -strip -background white -alpha off $SCAN_DIR/$j $SCAN_DIR/$j
...
#       is_blank=$(/usr/bin/docker run --rm -v "$SCANNER_BASE:/home/docker" img2pdf magick convert "$SCAN_DIR/$j" -format "%[fx:mean>0.
99?1:0]" info:)
...
### non blank list
find $SCANNER_BASE/$SCAN_DIR \( -name "*.tiff" -o -name "*.pnm" \) -printf "$SCAN_DIR/%f\n"| sort > $SCANNER_BASE/$SCAN_DIR/pdf_nonblan
k.txt
ret_code=$?
if [ $ret_code -ne 0 ]; then rm $SCANNER_BASE/$SCAN_DIR/pdf*.pdf ; exit 1; fi

### convert to a single pdf
docker run --rm -v "$SCANNER_BASE:/home/docker" img2pdf img2pdf -o $SCAN_DIR/pdf.pdf `cat $SCANNER_BASE/$SCAN_DIR/pdf_nonblank.txt`
ret_code=$?
if [ $ret_code -ne 0 ]; then rm $SCANNER_BASE/$SCAN_DIR/pdf*.pdf ; exit 1; fi

### ocr the pdf file
docker run --rm -v "$SCANNER_BASE:/home/docker" img2pdf ocrmypdf --deskew --rotate-pages --rotate-pages-threshold 3 $SCAN_DIR/pdf.pdf $
SCAN_DIR/pdf_output.pdf
ret_code=$?
if [ $ret_code -ne 0 ]; then rm $SCANNER_BASE/$SCAN_DIR/pdf*.pdf ; exit 1; fi
...
/usr/bin/docker exec nextcloud sudo -u abc php -d memory_limit=4096M /config/www/nextcloud/occ fulltextsearch:index "{\"user\":\"DOCUME
NTS\", \"providers\":\"files\", \"path\":\"SCANNER\"}"  > /dev/null 2>&1
